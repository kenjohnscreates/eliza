...
export class RuntimeModelDispatch {
	...
	registerModel(
		...
		void this.runtime
			...
			.catch((error) => {
				// error-policy:J7 diagnostics-must-not-kill-the-loop — `emitEvent`
				// awaits its handlers with a bare `Promise.all`, so any subscriber
				// (including a plugin's) that rejects would otherwise surface here
				// as an unhandled rejection on a fire-and-forget dispatch.
				this.runtime.reportError("ModelDispatcher.modelRegistered", error, {
					modelType: modelKey,
					provider,
				});
			});
